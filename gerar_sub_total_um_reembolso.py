...
class Sub_total_um_reembolso:
    ...
    def valida_funcionario_reembolso(self):   
        ...
        funcionario_reembolso_dict = CriarReembolso.carregar_funcionario_reembolso()
        print(f"Cargos disponíveis reembolso: {list(funcionario_reembolso_dict.keys())}")

        funcionario_normalizado = self.name_funcionario.strip().lower()
        # Normaliza o dicionário de cargos para comparação
        funcionario_dict_normalizado = {k.strip().lower(): v for k, v in funcionario_reembolso_dict.items()}

        if funcionario_normalizado in funcionario_dict_normalizado:
            self.funcionario_reembolso = funcionario_dict_normalizado[funcionario_normalizado]  # Armazena o objeto Funcionario
            return True
        else:
    ...
    def calcular_pagamento_um_reembolso(self)-> dict:
        if not self.valida_funcionario_reembolso():
        ...
        valor_hora_base = self.funcionario_reembolso['valor_hora_base']
        valor_repouso_remunerado = self.funcionario_reembolso['repouso_remunerado']
        valor_hora_extra_um = self.funcionario_reembolso['valor_hora_extra_um']
        valor_hora_extra_dois = self.funcionario_reembolso['valor_hora_extra_dois']
        adicional_noturno = self.funcionario_reembolso['adicional_noturno']
        valor_ferias = self.funcionario_reembolso['valor_ferias']
        valor_antecipa_ferias = self.funcionario_reembolso['valor_um_terco_ferias']
        valor_decimo_terceiro = self.funcionario_reembolso['valor_decimo_terceiro']
        pagamento_fgts = self.funcionario_reembolso['pagamento_fgts']
        desconto_inss = self.funcionario_reembolso['desconto_inss']
        desconto_refeicao = self.funcionario_reembolso['desconto_refeicao']
        desconto_transporte = self.funcionario_reembolso['desconto_transporte']
        ...
        sub_total_um_tres = sub_total_um * valor_decimo_terceiro /100
